...
export default function Home() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    // Stub for now — redirects to /profile
    router.push('/profile');
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-purple-900 to-blue-900 text-white">
      <div className="bg-white p-8 rounded-lg shadow-xl w-96 text-black">
        ...
        <p className="text-center text-gray-600 mb-6">Unlock your 3rd eye with stereoscopic training.</p>
        <p className="text-center text-gray-600 mb-6">Sign in or create an account to begin.</p>

        <div className="flex mb-6">
          <button
            onClick={() => setIsRegister(false)}
            className={`flex-1 py-2 text-center ${!isRegister ? 'bg-purple-600 text-white' : 'bg-gray-200 text-gray-600'}`}
          >
            Login
          </button>
          <button
        ...
        <p className="text-center text-sm text-gray-500 mt-4">
          Already have an account? {isRegister ? 'Log in' : 'Register'}
        </p>
      </div>
    </div>
  );
